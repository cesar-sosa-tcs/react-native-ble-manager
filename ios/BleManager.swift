...
class BleManager: RCTEventEmitter, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func handleDiscoveredPeripheral(_ peripheral: CBPeripheral,
        ...
        var services = peripheral.services?.map({ cb in
            return cb.uuid.uuidString
        })
        ...
        serialQueue.sync {
            if let p = peripherals[peripheral.uuidAsString()] {
                ...
                cp = Peripheral(peripheral:peripheral, rssi:rssi, advertisementData:advertisementData, services: services)
